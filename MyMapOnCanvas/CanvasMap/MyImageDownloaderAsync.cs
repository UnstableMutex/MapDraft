...
namespace CanvasMap
{
    internal static class MyImageDownloaderAsync
    {
        ...
        private static async Task<ImageSource> GetImage(byte zoom, int x, int y)
        {
        ...
        internal static bool IsIndexCorrect(int index, byte zoom)
        {
            var max = Math.Pow(2, zoom);
            return !(index > max - 1);
        }
