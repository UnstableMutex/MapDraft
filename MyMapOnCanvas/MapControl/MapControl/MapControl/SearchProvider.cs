...
namespace MapControl
{
    ...
    {
        private const string SearchPath = "http://geocode-maps.yandex.ru/1.x/?geocode={0}&format=xml&results={1}&lang={2}";
        ...
        public bool Search(string query, Rect area)
        {
            ...
            //client.QueryString.Add("geocode", Uri.EscapeDataString(query));
            // client.QueryString.Add("format", "xml");
            //client.QueryString.Add("results", "10");
            //client.QueryString.Add("lang", CultureInfo.CurrentCulture.EnglishName);
            var sp = string.Format(SearchPath, Uri.EscapeDataString(query),  "10",
                CultureInfo.CurrentCulture.Name);
         
            client.Proxy = WebRequest.DefaultWebProxy;
            client.Proxy.Credentials = CredentialCache.DefaultNetworkCredentials;
            client.Credentials = client.Proxy.Credentials;
            client.Encoding= Encoding.UTF8;
            //var s = client.DownloadString(sp);

            try
            {
                client.DownloadStringCompleted += this.OnDownloadStringCompleted;
                client.DownloadStringAsync(new Uri(sp));
            }
        ...
        private void OnDownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            ...
            try
            {
                var document = new GeoObjectCollection(e.Result);
                ...
                foreach (var node in document)
                {
                    ...
                    result.DisplayName = node.GeocoderMetaData.Text;// "sdfsdf";//node.Attributes.GetNamedItem("display_name").InnerText;
                    result.Latitude = node.Point.Lat;
                    result.Longitude = node.Point.Long;
